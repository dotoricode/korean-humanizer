...
- **정보량 보존.** humanizer 는 요약기가 아니다. 원문의 주장 / 근거 / 예시 / 뉘앙스 / 단락 순서를 유지한다. 문장이나 문단을 통째로 삭제하지 않는다. 출력 길이는 기본적으로 원문과 비슷해야 하며, 사용자가 "짧게"를 요청하지 않았다면 **원문 대비 90% 미만으로 줄이지 않는다.**
...
- **삭제 후보도 먼저 보존 가능성을 확인한다:** 인사 / 마무리 / 연결어라도 도메인상 역할이 있으면 짧게 다듬어 남긴다. 예: 이메일의 감사, 뉴스레터의 인사, YouTube의 호흡용 멘트.
...
- **압축 검토:** 원문보다 10% 이상 짧아졌거나 문장 수가 줄었다면, 삭제 / 병합한 부분을 다시 살린다. 사용자가 요약 / 축약 / 짧게 쓰기를 요청한 경우에만 이 제한을 완화한다.
...
- `references/ko-ai-signals.md` — 12 카테고리 / 100+ 패턴 카탈로그 (4 컬럼 — 나쁨 / 자연스러움 / 빈도 / 적용 도메인).
