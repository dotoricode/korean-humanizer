...
> 이 디렉토리는 데이터 기반 빈도 재라벨링을 위한 **스키마와 방법론만 저장소에 포함**한다. raw LLM 출력은 아직 수집되지 않았고, 현재 `high / med / low` 는 휴리스틱 라벨이다.
>
> raw 수집·계수·데이터 기반 재라벨링은 명시적으로 연기된 1.x 유지보수 작업이다. 외부 API 실행·모델 ToS 확인·원격 게시 여부는 이 README가 증명하지 않는 외부/수동 항목이다.

## 현재 상태

- v0.8 (S3 첫 패스): **샘플 수집 미완료**. 카탈로그의 빈도 라벨은 **휴리스틱 기반 v1** 유지.
- 후속 1.x sub-PR: 90 샘플 수집 → 137 패턴 빈도 카운팅 → 라벨 재산정 → 카탈로그 PR.
...
`scripts/count-frequencies.py` (1.x sub-PR 에서 작성 예정):
...
- 수집 시 90 샘플은 **공개 LLM API 의 자연 출력**만 사용하고, 사용자 데이터 / PII 는 넣지 않는다.
- 모델 ToS 의 학술·연구 목적 사용 허용 여부는 수집 전에 외부/수동으로 확인한다.
- 수집된 샘플은 git 에 commit 한다 (gitignore 안 함) — 재현성 확보.
...
이 작업은 카탈로그 v2 4 컬럼 구조 위에 얹히는 형태이므로 **1.x minor sub-PR** 로 분리. v1.0 freeze 약속에는 영향 없음.
