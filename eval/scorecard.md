<!-- Auto-generated by scripts/eval-harness.py — do not edit by hand. -->

# Eval scorecard

> Generated: 2026-04-30 02:03 UTC
> Source: `eval/fixtures/*.md` (25 fixtures)

## Summary

- Total fixtures: **25**
- Pass: **25** (100.0%)
- Fail: **0**

## Per-fixture results

| Fixture | Domain | M1 (mod ratio) | M2 (para cap) | M3 (length) | M4 (다체) | M5 (brand) | Overall |
|---|---|---|---|---|---|---|---|
| academic-01-abstract.md | academic | ✓ 0.0% (0/5) | ✓ max=0 | pass 0.96 | n/a (0→0) | n/a | ✓ |
| b2b-message-01-proposal.md | b2b-message | ✓ 16.7% (1/6) | ✓ max=1 | pass 0.87 | n/a (0→0) | n/a | ✓ |
| blog-01-time-management.md | blog | ✗ 100.0% (5/5) | ✗ max=5 | pass 0.61 | n/a (0→0) | n/a | ✓ (expected: m1,m2) |
| blog-02-quarterly-retro.md | blog | ✓ 17.8% (8/45) | ✓ max=2 | pass 0.92 | n/a (0→0) | n/a | ✓ |
| blog-03-remote-work.md | blog | ✗ 100.0% (6/6) | ✗ max=6 | pass 0.59 | n/a (0→0) | n/a | ✓ (expected: m1,m2) |
| blog-04-side-project.md | blog | ✓ 6.7% (1/15) | ✓ max=1 | pass 0.95 | n/a (0→0) | n/a | ✓ |
| brand-voice-01-toss-style-marketing.md | marketing | ✗ 100.0% (3/3) | ✓ max=3 | warn 0.34 | n/a (0→0) | pass (4/4) | ✓ (expected: m1,m3) |
| chat-01-kakao-biz.md | chat | ✗ 60.0% (3/5) | ✓ max=3 | pass 0.74 | n/a (0→0) | n/a | ✓ (expected: m1) |
| chat-02-dm-reply.md | chat | ✗ 60.0% (3/5) | ✓ max=3 | pass 0.66 | n/a (0→0) | n/a | ✓ (expected: m1) |
| edge-short-01-one-sentence.md | marketing | ✗ 100.0% (1/1) | ✓ max=1 | pass 0.78 | n/a (0→0) | n/a | ✓ (expected: m1) |
| edge-short-02-untouched.md | marketing | ✓ 0.0% (0/2) | ✓ max=0 | pass 1.00 | n/a (0→0) | n/a | ✓ |
| email-01-delay-apology.md | email | ✗ 66.7% (6/9) | ✗ max=6 | pass 0.72 | n/a (0→0) | n/a | ✓ (expected: m1,m2) |
| email-02-mtg-reschedule.md | email | ✗ 57.1% (4/7) | ✗ max=4 | pass 0.76 | n/a (0→0) | n/a | ✓ (expected: m1,m2) |
| email-03-vendor-followup.md | email | ✓ 10.0% (1/10) | ✓ max=1 | pass 0.92 | n/a (0→0) | n/a | ✓ |
| linkedin-01-series-a.md | linkedin | ✗ 100.0% (7/7) | ✗ max=7 | pass 0.69 | n/a (0→0) | n/a | ✓ (expected: m1,m2) |
| linkedin-02-product-update.md | linkedin | ✓ 14.3% (1/7) | ✓ max=1 | pass 0.92 | n/a (0→0) | n/a | ✓ |
| marketing-01-headphones.md | marketing | ✗ 100.0% (6/6) | ✗ max=6 | pass 0.59 | n/a (0→0) | n/a | ✓ (expected: m1,m2) |
| marketing-02-launch.md | marketing | ✗ 100.0% (4/4) | ✗ max=4 | pass 0.66 | n/a (0→0) | n/a | ✓ (expected: m1,m2) |
| marketing-03-feature-page.md | marketing | ✓ 0.0% (0/9) | ✓ max=0 | pass 0.91 | n/a (0→0) | n/a | ✓ |
| news-01-tech-coverage.md | news | ✓ 20.0% (1/5) | ✓ max=1 | pass 0.96 | n/a (0→0) | n/a | ✓ |
| newsletter-01-startup-seed.md | newsletter | ✗ 83.3% (5/6) | ✗ max=5 | pass 0.72 | n/a (0→0) | n/a | ✓ (expected: m1,m2) |
| review-01-product.md | review | ✗ 83.3% (5/6) | ✗ max=5 | pass 0.82 | n/a (0→0) | n/a | ✓ (expected: m1,m2) |
| wiki-01-korean-research.md | wiki | ✗ 42.9% (3/7) | ✓ max=3 | pass 0.88 | n/a (0→0) | n/a | ✓ (expected: m1) |
| youtube-01-time-mgmt.md | youtube | ✗ 83.3% (5/6) | ✗ max=5 | pass 0.52 | pass (2→0) | n/a | ✓ (expected: m1,m2) |
| youtube-02-trap-dache.md | youtube | ✓ 0.0% (0/6) | ✓ max=0 | pass 0.97 | fail (0→3) | n/a | ✓ (expected: m4) |

## Legend

- **M1**: modified sentence count / total raw sentences. ✓ = within `cap` (default 20%).
- **M2**: max modified sentences in any paragraph. ✓ = within `paragraph_cap` (default 3).
- **M3**: char-length ratio (humanized / raw). `pass` 0.5–1.05, `warn` 0.30–0.5 or 1.05–1.20, `fail` <0.30 or >1.20.
- **M4**: 다체 intrusion check. Active only for speech domains (youtube/podcast/live/lecture); else `n/a`.
- **M5**: brand voice `preserve` coverage. Active only when fixture frontmatter has `brand_voice: <path>`; else `n/a`. Format `pass (N/total)` = N preserved out of total preserve list.
- Overall ✗ marked `(expected: m4)` etc. for trap fixtures — not an actual regression.
